...
template <> int GfxUISensorFilter<float>::_render(UIGfxWrapper* ui_gfx) {
  ...
  if (_filter->dirty()) {
    ...
    if (underPointer()) {
      ...
      float v_scale = (v_max - v_min) / i_h;
      const uint32_t DATA_IDX = DATA_SIZE - (_pointer_x-(i_x+1));
      if (DATA_IDX < DATA_SIZE) {
        const float  DATA_VALUE = *(F_MEM_PTR + ((DATA_IDX + LAST_SIDX) % DATA_SIZE));

        uint32_t tmp_y = DATA_VALUE / v_scale;
        ui_gfx->img()->drawFastVLine(_pointer_x, i_y, i_h, _style.color_active);
        //ui_gfx->img()->fillRect((_pointer_x-1), (tmp_y-1), 3, 3, _style.color_active);
        ...
        ui_gfx->img()->getTextBounds(&temp_txt, 0, 0, &txt_x, &txt_y, &txt_w, &txt_h);
        if ((txt_w + txt_x) > (i_x + i_w)) {
          // If the string would overflow the element's right-hand boundary,
          //   render it on the left-hand side of the indicator line.
          txt_x = _pointer_x - txt_w;
