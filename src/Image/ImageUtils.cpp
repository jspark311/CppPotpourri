...
void UIGfxWrapper::_draw_graph_text_overlay(
  ...
  if (flags & GFXUI_FLAG_TEXT_VALUE) {
    ...
    _img->setCursor(x, strict_min((uint16_t) ((y+h)-tmp), (uint16_t) (h-1)));
...
*/
void UIGfxWrapper::draw_progress_bar_horizontal(
  int x, int y, int w, int h, uint16_t color,
...
* @param percent is in the range [0.0, 1.0]
*/
void UIGfxWrapper::draw_progress_bar_vertical(
  int x, int y, int w, int h, uint16_t color,
  bool draw_base, bool draw_val, float percent
) {
  if (draw_base) {   // Clear the way.
...
/*
*/
void UIGfxWrapper::draw_compass(
...
*/
void UIGfxWrapper::draw_3vector(
  int x, int y, int w, int h, uint16_t color,
...
*/
void UIGfxWrapper::draw_data_square_field(
  int x, int y, int w, int h,
  uint32_t flags,
